...
if jord_og_sol:
    ...
    plt.title("Jorda og Sola når startfarten til jorda er v = 8.9AU/år")
    plt.legend()
    plt.show()
...
if perihelion:
    ...
    theta = np.arctan(y/x) *3600#*206264.806  #arcsec
    theta1 = np.arctan(y1/x1)*3600 #*206264.806 #arcsec
    print_theta = theta-theta1
    #plt.plot(tid, print_theta)
    plt.plot(tid, theta, label="med ekstra kraft")
    plt.title("Perihelionvinkel")
    plt.xlabel("Tid [år]")
    plt.ylabel("Theta [arcsec]")
    plt.legend()
    plt.show()
    plt.plot(tid1, theta1, label="uten ekstra kraft")
    plt.title("Perihelionvinkel")
    plt.xlabel("Tid [år]")
    plt.ylabel("Theta[arcsec]")
    plt.legend()
