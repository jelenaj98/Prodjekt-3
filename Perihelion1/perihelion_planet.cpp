...
{   // Function that calculates the gravitational force between two objects, component by component.
    ...
    // Calculate the forces in each direction
      bool perihelion= false;
      if (perihelion){
      double c =  63239.7;//63241.1; // AU/yr
      double lx= ry*vz -rz*vy;
      double ly = ry*vx - rx*vz;
      double lz = rx*vy - ry*vx;
      double l = sqrt(lx*lx +ly*ly+lz*lz);

      //std::cout << (1. + (3*l*l)/(r*r*c*c)) << "    "<<r<< "    "<<l << "    "<<c<<'\n';
      Fx -= G*mass*other.mass*rx/((r*r*r) + smoothing)* (1+ (3*l*l*rx)/(r*r*r*c*c));
      Fy -= G*mass*other.mass*ry/((r*r*r) + smoothing)* (1+ (3*l*l*ry)/(r*r*r*c*c));
      Fz -= G*mass*other.mass*rz/((r*r*r) + smoothing)* (1+ (3*l*l*rz)/(r*r*r*c*c));
