...
Planet::Planet(string name,double M,double x,double y,double z,double vx, double vy,double vz){
    ...
    string filename = "pherilion_positions_1" + name + ".txt";
    posfile = fopen(filename.c_str(),"w");

    filename = "pherilion_energy_" + name + ".txt";
...
void Planet::PrintPosition(double time, double x, double y, double z)
...
void Planet::Perihelion(Planet &other, double time){
  ...
  if ((r_old2 > r_old1) && (r_old1< r_new)){
    ...
    PrintPosition(time, x_old1, y_old1, z_old1);
...
{   // Function that calculates the gravitational force between two objects, component by component.
    ...
    // Calculate the forces in each direction
      ...
      Fx -= G*mass*other.mass*rx/((r*r*r) + smoothing)* (1+ (3*lx*lx)/(rx*rx*c*c));
      Fy -= G*mass*other.mass*ry/((r*r*r) + smoothing)* (1+ (3*ly*ly)/(ry*ry*c*c));
      Fz -= G*mass*other.mass*rz/((r*r*r) + smoothing)* (1+ (3*lz*lz)/(rz*rz*c*c));
